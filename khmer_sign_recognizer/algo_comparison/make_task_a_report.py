...
ap.add_argument("--tag", default=None,
                help="suffix of the results folder, matching run_task_a.py")
ap.add_argument("--title", default=None, help="override the document title")
args = ap.parse_args()

OUT = ROOT / "algo_comparison" / (
    f"results_{args.lang}_taskA" + (f"_{args.tag}" if args.tag else ""))
...
# ── cover ────────────────────────────────────────────────────────────
h = doc.add_heading(args.title or "Task A — Khmer Sign Language Recognition",
                    level=0)
h.alignment = CENTER
cap = R.get("cap_per_sign")
# Only Task A was recorded on a condition grid. Saying otherwise would put a
# protocol on the cover that the corpus does not have.
if cap:
    _sub = f", capped at {cap} per sign"
elif R.get("condition_transfer"):
    _sub = f", on a {R['grid']}-condition grid"
else:
    _sub = ", recorded freely (no condition grid)"
para(doc, f"{R['n_real']} recordings of {len(R['labels'])} signs by "
          f"{len(R['signers'])} signers" + _sub,
...
if R.get("condition_transfer"):
    para(doc, f"Each of the {len(R['labels'])} signs was performed "
              ...
              f"standing positions. {len(R['signers'])} people recorded in "
              f"different rooms, giving {R['n_real']} real takes.")
else:
    para(doc, f"Each of the {len(R['labels'])} signs was performed about "
              f"{R['grid']} times by each of {len(R['signers'])} signers, "
              f"giving {R['n_real']} real takes. **There is no condition "
              f"grid** — signers recorded freely, in their own rooms and "
              f"lighting, rather than to a prescribed set of conditions.")
    if cap:
        para(doc, f"Some people recorded more than {cap} takes per sign. This "
                  f"report uses the first {cap} per signer per sign so the "
                  f"protocol is uniform; nothing was deleted, and the models "
                  f"shipped for use are trained on every take.")
para(doc, f"A further {R['n_synth']} synthetic takes were generated by "
          f"rebuilding each recording on differently-proportioned bodies.")
...
cond = R.get("conditions_per_sign") or {}
if cond:
    para(doc, "The important question is whether it fails under particular "
              "recording conditions — if so, the fix is more conditions. It "
              "does not:")
    rows = []
    for key, (ok, n) in sorted(cond.items()):
        lab, light, dist = key.split("|")
        if lab == weak and n:
            rows.append([f"{light} light, {dist}", f"{ok}/{n}",
                         f"{ok/n*100:.0f}%"])
    if rows:
        table(doc, ["Condition", "Correct", "Rate"], rows, [2.6, 1.4, 1.4])
        para(doc)
    para(doc, "The failures are spread across every lighting and distance "
              "combination, so this is a property of the sign pair rather "
              "than of the capture setup.")
else:
    para(doc, "This corpus was recorded freely — no lighting, distance or "
              "position was prescribed — so there is no condition to break "
              "the failures down by. What can be said is which signs are "
              "weakest and which pair they are confused with.")
para(doc, f"**The recommendation is more recordings of {NAME[weak]} and the "
          f"sign it is confused with — not more recordings in general.** "
          f"Every other sign is already high enough that additional takes of "
          f"them would change very little.")
...
if R.get("condition_transfer"):
    ...
    # Task B was recorded freestyle. Reporting a condition-transfer table
    # here would invent a protocol the corpus does not have.
    doc.add_heading("8. Recording conditions", level=1)
    para(doc, "This corpus has **no condition grid**. Signers recorded in "
              "their own rooms and lighting, with no prescribed set of "
              "lighting levels, distances or standing positions, so there is "
              "no condition to hold back and no transfer to measure.")
    para(doc, "That question was answered on the other corpus, which was "
              "recorded on a deliberate 12-cell grid. There, holding back a "
              "whole lighting condition cost almost nothing once several "
              "people had recorded — the variety arrived from having "
              "different people in different rooms rather than from "
              "controlling the conditions.")
    para(doc, "The freestyle recording used here is therefore not a weakness "
              "of this corpus: it is the same variety arriving informally.")
